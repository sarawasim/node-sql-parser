import { getSnowflakeFilters } from "../../src/snowflake/snowflakeParser.js"
...
describe("Snowflake parser", () => {
  it("should return no date filters when query does not include a date condition", () => {
    ...
    const dateFilters = getSnowflakeFilters(sqlQuery)

    expect(dateFilters).to.have.length(0)
  })

  // it("should parse YEAR(transaction_date) = YEAR(CURRENT_DATE()) AND MONTH(transaction_date) = MONTH(CURRENT_DATE()) as two 'current' month filters", () => {
  //   const sqlQuery =
  ...
  //   expect(dateFilters[0].type).to.be.eql("current")
  //   expect(dateFilters[0].numberOfPeriods).to.be.eql(1)
  //   expect(dateFilters[0].period).to.be.eql("year")
  //   expect(dateFilters[0].field).to.be.eql("transaction_date")
  // })

  // it("should parse YEAR(transaction_date) = YEAR(CURRENT_DATE()) AND MONTH(transaction_date) = MONTH(CURRENT_DATE()) as two 'current' filters", () => {
  //   const sqlQuery =
  //     "SELECT * FROM transactions WHERE YEAR(transaction_date) = YEAR(CURRENT_DATE()) AND MONTH(transaction_date) = MONTH(CURRENT_DATE())"
  ...
  // it("should parse transaction_date >= DATEADD(YEAR, -1, CURRENT_DATE()) as a 'current' year filter", () => {
  //   const sqlQuery =
  ...
  it("should parsetransaction_date >= CURRENT_DATE() - INTERVAL '90 DAY' as a 'last' 90 day filter", () => {
    const sqlQuery =
  ...
  it("should parse transaction_date >= CURRENT_DATE() - INTERVAL '90 DAY' as a 'last' 6 month filter", () => {
    const sqlQuery =
  ...
  // it("should parse transaction_date >= DATE_TRUNC('MONTH', CURRENT_DATE() - INTERVAL '1 MONTH') AND transaction_date < DATE_TRUNC('MONTH', CURRENT_DATE()) as two 'previous' filters", () => {
  //   const sqlQuery =
  ...
  // it("should parse transaction_date >= DATEADD(month, -1, DATE_TRUNC('MONTH', CURRENT_DATE())) AND transaction_date < DATE_TRUNC('MONTH', CURRENT_DATE()) as two 'previous' filters", () => {
  //   const sqlQuery =
