...
const { Parser } = pkg
import { Binary, Function, Parser } from "node-sql-parser"

export function getBigQueryFilters(sqlQuery: string): DateFilter[] {
  const parser = new Parser()
  const ast = parser.astify(sqlQuery, { database: "bigquery" })
...
function exploreFunctionExpression(expression: Function) {
  const functionName =
    expression.args.value[0]?.name.name[0].value.toLowerCase()
...
function parseDateFilter(exprParent, period): DateFilter | null {
  ...
  period = period
    ? period
    : getPeriodFormatted(getPeriod(exprParent)?.toLowerCase())
...
function getType(expr: any, operator: string): string {
  const isCurrentDate = (side: any) =>
    side?.type === "function" &&
    side?.args?.value[0]?.name?.name[0].value.toLowerCase() === "current_date"

  const isDateSubFunction = (side: any) =>
    side?.type === "function" &&
    ["date_sub", "year", "month", "date_trunc"].includes(
      side?.name?.schema?.value?.toLowerCase()
    )
...
function getNumberOfPeriods(expr: any) {
  ...
  if (expr.right.type === "function") {
    if (expr.right.name.schema.value.toLowerCase() === "date_sub") {
...
function getPeriod(expr: any) {
  if (expr.type === "binary_expr") {
    ...
    if (expr.right.type === "function") {
      if (expr.right.name.schema.value.toLowerCase() === "date_sub") {
        const intervalString = expr.right.args.value[1].unit.toLowerCase()
        return getPeriodFormatted(intervalString)
      }
    }
  } else if (expr.type === "function") {
    if (expr.name.name[0].value.toLowerCase() === "date_trunc") {
